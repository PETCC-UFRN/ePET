...
import javafx.scene.control.TabPane;import javafx.scene.control.TableColumn;
...
public class TelaUsuarioPrincipal {
    ...
    public void popularEventosAbertos() throws Exception{
    	...
    	try (Response response = httpClient.newCall(request).execute()) {
            ...
            if (!response.isSuccessful()) {//throw new IOException(response.body().toString());
        		...
        		//int tam = response.body().string().length();
        		//System.out.println(response.body().string().substring(1));
        		...
        		/*
        		Callback<TableColumn<Evento, Boolean>,TableCell<Evento, Boolean>> aux = CheckBoxTableCell.forTableColumn(abertosInscrever);
        		aux.call(abertosInscrever).setOnKeyPressed(new EventHandler< Event>(){
					@Override
        		...
        		abertosInscrever.setCellFactory(aux);
        		//abertosInscrever.setOnEditStart();
        		
        		abertosInscrever.setCellFactory(new Callback<TableColumn<Evento, Boolean>, TableCell<Evento, Boolean>>() {
        		      @Override public TableCell<Evento, Boolean> call(TableColumn<Evento, Boolean> eventosTableColumn) {
        		        return new AddEventoCell(tableAbertos);
        		      }
        		    });*/
    ...
    public void popularParticipantes() throws Exception{
    	...
    	try (Response response = httpClient.newCall(request).execute()) {
            ...
            if (!response.isSuccessful()) {//throw new IOException(response.body().toString());
        		...
        		System.out.println(jsonArray);
        		/*
        		participanteCH.setCellValueFactory(new PropertyValueFactory<>(("participanteCH")));
        		participanteDataInicio.setCellValueFactory(new PropertyValueFactory<>(("participanteDataInicio")));
        		participanteDescricao.setCellValueFactory(new PropertyValueFactory<>(("participanteDescricao")));
        		//participanteId.setCellValueFactory(new PropertyValueFactory<>(("participanteId")));
        		participanteEventoId.setCellValueFactory(new PropertyValueFactory<>(("participanteEventoId")));
        		participanteConfirmado.setCellValueFactory(new PropertyValueFactory<>(("participanteConfirmado")));
        		participanteConfirmado.setCellFactory(CheckBoxTableCell.forTableColumn(participanteConfirmado));
        		participanteLocal.setCellValueFactory(new PropertyValueFactory<>(("participanteLocal")));
        		participanteTitulo.setCellValueFactory(new PropertyValueFactory<>(("participanteTitulo")));
        		participanteVagas.setCellValueFactory(new PropertyValueFactory<>(("participanteVagas")));
        		participanteValor.setCellValueFactory(new PropertyValueFactory<>(("participanteValor")));
        		//participanteRetirar.setCellValueFactory(new PropertyValueFactory<>(("participanteRetirar")));
        		//participanteRetirar.setCellFactory(CheckBoxTableCell.forTableColumn(participanteRetirar));

        		List<Evento> listaDeParticipante = new ArrayList<>();
        		for (int i = 0; i < jsonArray.length(); i++) {
        			JSONObject json = new JSONObject(jsonArray.get(i).toString());
        			//System.out.println(json.getJSONObject("evento"));
        			System.out.println(json.getJSONObject("evento").getDouble("qtdCargaHoraria"));
        			listaDeParticipante.add(new Evento(json.getJSONObject("evento").getInt("idEvento"),
        					json.getJSONObject("evento").getString("titulo"),
        					json.getJSONObject("evento").getString("descricao"),
        					json.getJSONObject("evento").getString("local"),
        					json.getJSONObject("evento").getInt("qtdVagas"),
        					json.getJSONObject("evento").getDouble("qtdCargaHoraria"),
        					json.getJSONObject("evento").getDouble("valor"),
        					json.getJSONObject("evento").getString("d_inscricao"), false 
        					//json.getBoolean("confirmado")//, json.getBoolean("espera"),
        					//false, json.getInt("idParticipantes")
        					));
				}
        		tableParticipante.setItems(FXCollections.observableArrayList(listaDeParticipante));
        		*/
        		...
        		for (int i = 0; i < jsonArray.length(); i++) {
        			...
        			//System.out.println(json.getJSONObject("evento"));
        			//System.out.println(json.getJSONObject("evento").getDouble("valor"));
        			listaDeParticipante.add(new Evento(json1.getInt("idParticipantes"),
        					...
        					/*add(new Evento(json.getJSONObject("evento").getInt("idEvento"),
        					json.getJSONObject("evento").getString("titulo"),
        					json.getJSONObject("evento").getString("descricao"),
        					json.getJSONObject("evento").getString("local"),
        					json.getJSONObject("evento").getInt("qtdVagas"),
            ...
            if (!response.isSuccessful()) {//throw new IOException(response.body().toString());
				...
				for (int i = 0; i < jsonArray.length(); ++i) {
					try {
						...
						if (tutoria.getJSONObject("pessoa").getInt("idPessoa") == mainApplet.getIdPessoa() )
						{
							listaTutoriasMinistradas.add(new TutoriaMinistrada(tutoria.getInt("idTutoria_ministrada"),
									...
									tutoria.getJSONObject("tutoria").getJSONObject("petiano").getJSONObject("pessoa").getString("nome"),
    ...
    /*
    private class AddEventoCell extends TableCell<Evento, Boolean> {
        ...
        AddEventoCell(final TableView table) {
