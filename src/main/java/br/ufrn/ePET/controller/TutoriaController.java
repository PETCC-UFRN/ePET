...
public class TutoriaController {
	...
	//@Secured({"ROLE_tutor", "ROLE_comum", "Role_admin"})
	public ResponseEntity<?> getTutorias(Pageable pageable){
		Page<TutoriaOnline> tutorias = tutoriaService.buscar(pageable);
		if(tutorias.isEmpty()) {
	...
	public ResponseEntity<?> getTutoria(@PathVariable Long id){
		TutoriaOnline tutoria = tutoriaService.buscar(id);
		if(tutoria == null) {
	...
	public ResponseEntity<?> saveTutoria(@PathVariable Long id_petiano, @PathVariable Long id_disciplina, @RequestBody TutoriaOnline tutoria_online){
		tutoriaService.salvar(id_petiano, id_disciplina, tutoria_online);
	...
	@PostMapping(value="/tutoria-avaliar/{id}/{nota}")
	//@Secured({"ROLE_tutor", "Role_admin"})
	public ResponseEntity<?> saveTutoria(@PathVariable Long id, @PathVariable Double nota){

		TutoriaOnline tutoria = tutoriaService.buscar(id);
		if(tutoria == null) {
			throw new ResourceNotFoundException("Tutoria com o id: " + id + " não foi encontrada.");
		}
		tutoriaService.avaliar(tutoria, nota);
		//tutoriaService.salvar(id_petiano, id_disciplina, tutoria_online);
		return new ResponseEntity<>(HttpStatus.OK);
	}
