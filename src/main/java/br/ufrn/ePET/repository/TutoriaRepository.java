...
public interface TutoriaRepository extends JpaRepository<Tutoria, Long>{
	...
	@Query(value = "SELECT t.id_tutoria, t.id_disciplina, t.id_petiano, t.ativo FROM tutoria t INNER JOIN disciplina d ON d.id_disciplina = t.id_disciplina WHERE (d.codigo LIKE %?1% OR d.nome LIKE %?1%) AND t.ativo = 0", nativeQuery = true)
