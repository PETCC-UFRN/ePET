...
public interface OrganizadoresRepository extends JpaRepository<Organizadores, Long> {
	...
	@Query(value = "SELECT * FROM organizdores u WHERE u.id_pessoa = ?2 AND u.id_evento = ?1", nativeQuery = true)
	Organizadores findByPessoaAndEvento(Long id_evento, Long id_pessoa);
}
